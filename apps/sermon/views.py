...
def index(request):
    ...
    if today_sermon:
        ...
        similar_sermons = Sermon.objects.filter(tags__in=today_sermon_tags_ids).exclude(
            id=today_sermon.id
        )
        similar_sermons = similar_sermons.annotate(same_tags=Count("tags")).order_by(
            "-same_tags", "-datetime_created"
        )[:2]
        ...
        context = {
            ...
            "similar_sermons": similar_sermons,
            "total_views": total_views,
            "popular_sermons": popular_sermons,
        }
...
def sermon_pdf(request, id):
    ...
    weasyprint.HTML(string=html, base_url=request.build_absolute_uri()).write_pdf(
        response,
        stylesheets=[
            weasyprint.CSS(settings.STATIC_ROOT + "sermon/css/sermon.css"),
